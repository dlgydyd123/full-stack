...
function MyMap2() {
    ...
    //사용자가 클릭한 마커의 정보를 저장하는 state
    const [info, setInfo] = useState(null);

    //지도에 표시될 마커들의 리스트 state
    const [markers, setMarkers] = useState([]);

    //생성된 카카오 맵 객체를 저장하는 state
    ...
    const searchPlaces = (searchKeyword) => {
        //map 객체과 카카오 지도 API가 로드되지 않으면 함수 종료
    ...
    return (
        <div>
            ...
            <Map
                center={{
                    lat: 37.492177797688356,
                    lng: 126.72397165063684
                }}
                style={{
                    width: '400px',
                    height: '400px'
                ...
                //매개변수로 받아 그 객체에 대한 접근 및 추가설정을 가능하게 한다.
            >
                {markers.map((marker, index) =>(
                    ...
                    >
                        ...
                        {/* 마우스를 올렸을 때 표시 */}
                        {hoverInfo === marker &&(
                            <div style={{color:"#000"}}>{marker.content.name}</div>
                        )}
